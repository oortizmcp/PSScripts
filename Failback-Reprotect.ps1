...
$priFabric = Get-AzRecoveryServicesAsrFabric | where {$_.FabricSpecificDetails.Location -like $RecoveryRegion -or $_.FabricSpecificDetails.Location -like $RecoveryRegion.Replace(' ', '')}
$priContainer = Get-AzRecoveryServicesAsrProtectionContainer -Fabric $priFabric
$recFabric = Get-AzRecoveryServicesAsrFabric | where {$_.FabricSpecificDetails.Location -like $PrimaryRegion -or $_.FabricSpecificDetails.Location -like $PrimaryRegion.Replace(' ', '')}
...
foreach ($reverseReplciationJob in $reverseReplicationJobs) {
    ...
    do {
        Start-Sleep -Seconds 50
        $job = Get-AzRecoveryServicesAsrJob -Job $reverseReplciationJob
        Write-Output $job.State
    } while ($job.State -ne 'Succeeded' -and $job.State -ne 'Failed' -and $job.State -ne 'CompletedWithInformation')
...
$rpisInContainer = Get-AzRecoveryServicesAsrReplicationProtectedItem -ProtectionContainer $recContainer | where {$sourceVmARMIds -contains $_.ProviderSpecificDetails.FabricObjectId}
